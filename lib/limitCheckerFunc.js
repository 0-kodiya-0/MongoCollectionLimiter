 ...
 *************************************************THIS IS IMPORTANT*************************************************
 ...
 2) Collection limit goes to the "collection limit" (col_Names[0][1] === collection limit) 
   ...
   * You need to set the "collection limit" in the collection as a document with the id that you specified in the ( "CONF_ID" ) env variable.
     And you should add the limit (that means the num) in the document with corresponding variable "documentLimit"
       ...
       The document in the collection should look like this
                                                            {
                                                             ...
                                                             documentLimit: Any limit here. ( Note:- the num should be a integer not a string )
                                                            }
   ...
   * When in the boot up it will read the id (that means the id that you specified in CONF_ID env variable) from the collection and 
     check the documentLimit variable and add the value to col_Names[0][1]. If null it will add null.

   * When you update documentLimit variable in the CONF document it will also update the col_Names[0][1] num. 
                ...
                if (data.ns.coll[0] === process.env.COLL_IDENTIFY_VALUE) {
                    if (data.documentKey._id === process.env.CONF_ID) { // This if state check the inserting value is a CONF doc or not
                        ...
                        if (col_Names.length === 0) { // This will run if there is not data in col_Names
                            ...
                            col_Names[0][1] = data.fullDocument.documentLimit; // This will add the collection limit  
                            ...
                            for (let i = 0; i < col_Names.length; i++) {
                                if (col_Names.length === i + 1 && col_Names[i][0] !== data.ns.coll && max_Collection_Limit > col_Names.length) {
                                    ...
                                    col_Names[col_Names.length - 1][1] = data.fullDocument.documentLimit;
                                    ...
                                    if (col_Names[i][1] === null || data.fullDocument.documentLimit !== col_Names[i][1]) { // This will run if the col_Names[i][1] is null and it will add if it is null
                                        console.log(`${data.ns.coll.slice(1, data.ns.coll.length)} collection ok . But ${data.ns.coll.slice(1, data.ns.coll.length)} documentLimit === null`);
                                        col_Names[i][1] = data.fullDocument.documentLimit;
                                        ...
                                        console.log(`${data.ns.coll.slice(1, data.ns.coll.length)} added documentLimit ${col_Names[i][1]} && added DocumentNum ${col_Names[i][2]}`)
                                        break;
                                    } else {
                ...
                if (data.documentKey._id === process.env.CONF_ID) { // This will run if a update happen to a CONF document
                    for (let i = 0; i < col_Names.length; i++) {
                        if (col_Names[i][0] === data.ns.coll) {
                            if (data.updateDescription.updatedFields.documentLimit !== col_Names[i][1]) {
                                col_Names[i][1] = data.updateDescription.updatedFields.documentLimit; // This will update documentLimit with updated collection limit
